...
- 用 Alpine.js 替代 jQuery/Bootstrap JS 完成交互，保持 SSR 兼容
...
5. `cps/static/js/` 关键行为逐步搬到 Alpine：
   - 导航抽屉：`x-show` 控制 Sidebar / Drawer
   - 模态框：`x-show` + `x-on:keydown.esc` 关闭
   - 批量选择：`x-data` 维护 `selected` 数组，`x-on` 处理 checkbox
   - 表单提交：保留原有 `form` 提交，但取消 `onclick` 中的 jQuery `$.ajax`，改用原生 `form.submit()` 或 `fetch` 配合 Alpine 状态
...
- Alpine.js 行为基本成立（Sidebar 抽屉可开关，主题切换有效）
